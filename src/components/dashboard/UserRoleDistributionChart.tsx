...
const roleOrder = ["alumno", "representante", "juez", "master", "admin"];
...
const mockUsers = mockAthletes.map(athlete => {
  const roles: ("admin" | "master" | "alumno" | "representante" | "juez")[] = ["alumno", "representante", "master", "juez", "admin"];
  const role = roles[athlete.id % 5] || 'alumno';
  return {
    id: athlete.id,
...
export default function UserRoleDistributionChart() {
  ...
  const { chartData, chartConfig } = useMemo(() => {
    const roleCounts: { [key: string]: number } = { admin: 0, master: 0, alumno: 0, representante: 0, juez: 0 };
