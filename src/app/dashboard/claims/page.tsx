...
export default function ClaimsPage() {
  ...
  const filteredClaims = useMemo(() => {
    ...
    if (hasRole('representante') && !hasRole(['admin', 'master'])) {
      userClaims = claimsData.filter(claim => claim.representativeId === user.id);
    } else if (hasRole(['admin', 'master'])) {
  ...
  }, [claimsData, searchTerm, user, hasRole]);
  ...
  const canManageClaims = hasRole(['admin', 'master']);
  const canCreateClaim = hasRole(['admin', 'master', 'representante']);

  const canEditOrDeleteClaim = (claim: Claim) => {
    if (!user) return false;
    return hasRole(['admin', 'master']) || user.id.toString() === claim.representativeId;
  }
