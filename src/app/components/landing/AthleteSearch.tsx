...
export default function AthleteSearch({ initialSearchTerm, onSearchChange }: { initialSearchTerm: string, onSearchChange: (term: string) => void }) {
  ...
  // Cambio 2: useEffect para marcar el componente como montado en el cliente
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  // --- CAMBIO 3: Renderizado condicional para evitar el error de hidratación ---
  if (!mounted) {
    // Mientras el componente no esté montado en el navegador, 
    // devolvemos null o un estado de carga básico que coincida con el servidor.
    return <div className="h-96 w-full animate-pulse bg-muted/20 rounded-lg" />;
  }
